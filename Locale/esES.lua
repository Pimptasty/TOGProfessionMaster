-- TOG Profession Master -- Spanish (esES) locale
-- Any missing key falls back to enUS automatically via AceLocale.
-- Translations are best-effort; native-speaker review welcome.

local _, addon = ...
local L = LibStub("AceLocale-3.0"):NewLocale("TOGProfessionMaster", "esES")
if not L then return end

-- Spanish (Latin America) shares this locale via AceLocale's enUS fallback,
-- but we also register esMX so esMX clients pick up these strings directly.
do
    local LMX = LibStub("AceLocale-3.0"):NewLocale("TOGProfessionMaster", "esMX")
    if LMX then
        -- Will be populated below alongside L (alias both tables).
        addon._esMX = LMX
    end
end
...
-- ---------------------------------------------------------------------------
-- Mirror everything we just set into the esMX locale table so esMX clients
-- (Latin-American Spanish, same Blizzard glossary) pick these up directly
-- without relying on AceLocale's enUS fallback.
-- ---------------------------------------------------------------------------
if addon._esMX then
    for k, v in pairs(L) do addon._esMX[k] = v end
    addon._esMX = nil
end
