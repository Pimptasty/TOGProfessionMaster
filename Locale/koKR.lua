...
L["SettingsSyncHeader"]     = "동기화"
L["SettingsGuildMode"]      = "길드 전용 동기화 모드 (사설 서버)"
L["SettingsGuildModeDesc"]  = "귓속말로 애드온 메시지를 전달하지 않는 사설 또는 에뮬레이트 서버(예: Whitemane)를 위한 옵션으로, 이 경우 길드원들이 서로의 전문기술 정보를 받지 못합니다. 켜면 모든 동기화 트래픽이 대신 길드 채널로 전송됩니다. 서버에서 길드 동기화가 작동하지 않을 때만 켜십시오. |cffffd100길드원 모두가 켜야 합니다|r — 양쪽 모두 켠 구성원 사이에서만 작동합니다. 이 옵션이 켜져 있는 동안에는 길드 간 공유가 자동으로 비활성화됩니다. 이 서버의 모든 캐릭터에 적용됩니다. (설치된 DeltaSync 버전이 지원하지 않으면 숨겨집니다.)"
...
L["SettingsCrafterAlertSuppressAudio"]     = "알림 소리 억제"
L["SettingsCrafterAlertSuppressAudioDesc"] = "제작자가 접속할 때 소리를 비활성화합니다(화면 깜빡임과 채팅 메시지는 여전히 나타남)."
L["SettingsCrafterAlertSuppressVisual"]    = "알림 시각 효과 억제"
L["SettingsCrafterAlertSuppressVisualDesc"] = "제작자가 접속할 때 화면에 표시되는 시각 알림(화면 깜빡임, 배너 등)을 비활성화합니다(소리와 채팅 메시지는 여전히 나타남)."
L["AlertCrafterOnlineBanner"]              = "길드 제작자 접속"
L["SettingsCrafterAlertSound"]             = "알림 소리"
L["SettingsCrafterAlertSoundDesc"]         = "연락 가능한 제작자가 접속했을 때 재생할 소리입니다. 선택하면 미리 들어볼 수 있습니다. 위에서 알림 소리를 억제한 경우에는 적용되지 않습니다."
L["SettingsCrafterAlertVisual"]            = "알림 시각 효과"
L["SettingsCrafterAlertVisualDesc"]        = "제작자가 접속할 때 발동되는 화면 효과입니다 — 선택한 색조의 전체 화면 깜빡임, 또는 다른 창을 보고 있을 때를 위한 작업 표시줄 깜빡임 중에서 선택합니다. 선택하면 미리 볼 수 있습니다. 위에서 화면 깜빡임을 억제한 경우에는 적용되지 않습니다."
